...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Toggle the boosted flag
    const newBoostedValue = !application.boosted;
    ...
    await db.collection('students').updateOne(
      ...
      {
        $set: { 
          'applications.$.boosted': newBoostedValue,
          updatedAt: new Date()
        }
      } as any
    );
    ...
    await db.collection('jobs').updateOne(
      ...
      {
        $set: { 
          'applicants.$.boosted': newBoostedValue
        }
      } as any
    );
    ...
    return NextResponse.json({ 
      success: true, 
      boosted: newBoostedValue,
      message: newBoostedValue ? 'Application boosted' : 'Application boost removed'
    });
  } catch (error) {
    console.error('Error toggling boost:', error);
    return NextResponse.json(
      { error: 'Failed to toggle boost' },
      { status: 500 }
    );
  }
}
